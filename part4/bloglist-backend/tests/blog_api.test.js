...
describe('when there are initially some blogs saved', () => {
    ...
    test('unique identifier property of blog posts is named "id"', async () => {
        ...
        assert.strictEqual(typeof blog.id, 'string', 'Blog post should have an "id" property')
    ...
    test('likes default to 0 if not provided in blog post', async () => {
        ...
        const blogsAtEnd = await helper.blogsInDb()
        const addedBlog = blogsAtEnd.find(b => b.title === 'blog without likes property')
        assert.strictEqual(addedBlog.likes, 0, 'likes should default to 0 when not specified')
    })
    ...
    test('updating the likes of a blog', async () => {
        ...
        assert.strictEqual(updatedBlog.likes, blogToUpdate.likes + 1, 'Likes should be incremented by 1')
    })
})
...
describe('when there is initially one user in db', () => {
    ...
    test('fails with status code 400 if username is missing', async () => {
        ...
        const usersAtEnd = await helper.usersInDb()
        assert(result.body.error, 'username of at least 3 characters must be provided')
